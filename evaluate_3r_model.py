...
def get_mast3r_output_single_file(model, input_path, output_path, device = 'cuda'):
    ...
    if re_proj:
        ...
        # 把点重新投回3D
        fx, fy, cx, cy = origin_intrinsics_params
        prediction_tracks_xyz[..., 0] = (u - cx) / fx * prediction_tracks_xyz[..., 2]
        prediction_tracks_xyz[..., 1] = (v - cy) / fy * prediction_tracks_xyz[..., 2]
    
    if normalize:
        gt_tracks_norm_factor = np.median(np.linalg.norm(tracks_xyz, axis = -1),axis = -1)
    ...
    if re_proj == False:
        origin_intrinsics_params = intrinsics_params   
