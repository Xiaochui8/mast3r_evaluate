...
def get_image_pairs(images_jpeg_bytes, query_idx, size=512, save_imgs=False, factor = 16):
    imgs = []
    img_pairs = np.empty((len(images_jpeg_bytes) - 1), dtype=tuple)
    idx = 0
    origin_shape = ()
    current_shape = ()
    ...
    for i in range(len(imgs) - 1):
            img_pairs[i] = tuple([imgs[i + 1], imgs[i]])
...
def get_mast3r_output_single_folder(input_path, output_path, device = 'cuda'):
    weights_path = "/mnt/nas/share/home/tjy/mast3r_evaluate/checkpoints/MASt3R_ViTLarge_BaseDecoder_512_catmlpdpt_metric.pth"
...
def get_mast3r_output_single_file(model, input_path, output_path, device = 'cuda'):
    ...
    output = inference(list(image_pairs), model, device, batch_size=32, verbose=False)
    
    query_2d = queries_xyt
    query_3d = np.zeros((query_2d.shape[0], 4))
    query_3d = query_3d[:, :3]
    for indx in range(t - 1):
        pts1 = output['pred1']['pts3d'][indx]
        pts2 = output['pred2']['pts3d_in_other_view'][indx]
        mask = query_2d[:, 2] == indx
        x, y = query_2d[mask, 0] / origin_shape[0] * current_shape[0], query_2d[mask, 1] / origin_shape[1] * current_shape[1]
        prediction_tracks_xyz[:, mask] = interpolate_at_xy(pts2, x, y).permute(0, 2, 1)
